...
To walk a filesystem (or directory) you can construct a :class:`~fs3.walk.Walker` object and use its methods to do the walking. Here's an example that prints the path to every Python file in your projects directory::
...
Note that the ``files`` method above doesn't require a ``fs`` parameter. This is because the ``walk`` attribute is a property which returns a :class:`~fs3.walk.BoundWalker` object, which associates the filesystem with a walker.

Walk Methods
~~~~~~~~~~~~

If you call the ``walk`` attribute on a :class:`~fs3.walk.BoundWalker` it will return an iterable of :class:`~fs3.walk.Step` named tuples with three values; a path to the directory, a list of :class:`~fs3.info.Info` objects for directories, and a list of :class:`~fs3.info.Info` objects for the files. Here's an example::
...
.. note ::
    Methods of  :class:`~fs3.walk.BoundWalker` invoke a corresponding method on a :class:`~fs3.walk.Walker` object, with the *bound* filesystem.

The ``walk`` attribute may appear to be a method, but is in fact a callable object. It supports other convenient methods that supply different information from the walk. For instance, :meth:`~fs3.walk.BoundWalker.files`, which returns an iterable of file paths. Here's an example::

    for path in home_fs.walk.files(filter=['*.py']):
        print('Python file: {}'.format(path))

The complement to ``files`` is :meth:`~fs3.walk.BoundWalker.dirs` which returns paths to just the directories (and ignoring the files). Here's an example::

    for dir_path in home_fs.walk.dirs():
        print("{!r} contains sub-directory {}".format(home_fs, dir_path))

The :meth:`~fs3.walk.BoundWalker.info` method returns a generator of tuples containing a path and an :class:`~fs3.info.Info` object. You can use the ``is_dir`` attribute to know if the path refers to a directory or file. Here's an example::
