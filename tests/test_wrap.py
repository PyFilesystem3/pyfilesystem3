...
class TestWrapReadOnlySyspath(unittest.TestCase):
    ...
    # in fs3.copy or fs3.mirror we try to use it to our advantage, but
    ...
    def test_copy_fs(self):
        self.assertReadOnly(fs3.copy.copy_fs, self.src, self.ro)

    def test_copy_fs_if_newer(self):
        self.assertReadOnly(lambda src_fs, dst_fs: fs3.copy.copy_fs_if(src_fs, dst_fs, condition="newer"), \
                            self.src, self.ro)

    def test_copy_file(self):
        self.assertReadOnly(fs3.copy.copy_file, self.src, "foo", self.ro, "foo")

    def test_copy_file_if_newer(self):
        self.assertReadOnly(lambda src_fs, src_path, dst_fs, dst_path: \
                            fs3.copy.copy_file_if(src_fs, src_path, dst_fs, dst_path, condition="newer"), \
                                self.src, "foo", self.ro, "foo")

    def test_copy_structure(self):
        self.assertReadOnly(fs3.copy.copy_structure, self.src, self.ro)

    def test_mirror(self):
        self.assertReadOnly(fs3.mirror.mirror, self.src, self.ro)
        fs3.mirror.mirror(self.src, self.fs)
        self.fs.touch("baz")
        self.assertReadOnly(fs3.mirror.mirror, self.src, self.ro)

    def test_move_fs(self):
        self.assertReadOnly(fs3.move.move_fs, self.src, self.ro)
        self.src.removetree("/")
        self.fs.touch("foo")
        self.assertReadOnly(fs3.move.move_fs, self.ro, self.src)

    def test_move_file(self):
        self.assertReadOnly(fs3.move.move_file, self.src, "foo", self.ro, "foo")
        self.fs.touch("baz")
        self.assertReadOnly(fs3.move.move_file, self.ro, "baz", self.src, "foo")

    def test_move_dir(self):
        self.assertReadOnly(fs3.move.move_file, self.src, "bar", self.ro, "bar")
        self.fs.makedir("baz")
        self.assertReadOnly(fs3.move.move_dir, self.ro, "baz", self.src, "baz")
...
class TestWrapCachedDir(unittest.TestCase):
    def setUp(self):
        ...
        self.fs.touch("egg")
        self.cached = fs3.wrap.cache_directory(self.fs)

    def tearDown(self):
        self.fs.close()

    def assertNotFound(self, func, *args, **kwargs):
        self.assertRaises(fs3.errors.ResourceNotFound, func, *args, **kwargs)
